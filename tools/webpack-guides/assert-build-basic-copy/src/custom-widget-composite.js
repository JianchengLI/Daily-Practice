(function() {
    ...
    class Composite {
        ...
        onReady() {}
        onSend() {}
        destroy() { rap.off("send", this.onSend); }
    ...
    class TileFontIcon extends Tile {
        ...
        redraw() {
            if (this.icon_only) {
                if (this.url == null)
                    this.element.innerHTML = "<div><i class=\"fa " + this.icon + "\" aria-hidden=\"true\"></i></div>";
                else {
                ...
                if (this.vertical) {
                    if (this.url == null)
                        this.element.innerHTML = "<div><i class=\"fa " + this.icon + "\" aria-hidden=\"true\"></i><br />" + this.name + "</div>";
                    else
                        this.element.innerHTML = "<div><a href=\"" + this.url + "\" target=\"_blank\">" + "<i class=\"fa " + this.icon + "\" aria-hidden=\"true\"></i><br />" + this.name; + "</a></div>"
                }
            }
        }
    }
    ...
    class TileSimple extends Tile {
        redraw() {
            if (this.bg_image_src) {
                ...
                this.element.innerHTML = this.url ?
                    "<a href=\"" + this.url + "\" target=\"_blank\"><b>" + this.name + "</a></b>" : "<b>" + this.name + "</b>";
            }
        }
    }
    ...
    class TileCharts extends Tile {
        ...
        templateRowChart(container_id, container_info_id, chart, index) {
            return "<div id=\"" + container_id + "_" + index + "\">" +
                "</div>" +
                "<div id=\"" + container_info_id + "_" + index + "\" class=\"chart_info\">" +
                "<div class=\"chart_info_title\">" +
                chart.indiceTitle + "</div>" +
                "<div class=\"chart_info_value\">" +
                chart.data[0].value + " " + chart.indiceUnitName + "</div>" +
                "</div>";
        }
    ...
    class MenuSelect extends Menu {
        ...
        redraw(jsonObject) {
            this.element.innerHTML = "<i class=\"fa fa-angle-right\" aria-hidden=\"true\"></i><div class=\"ClientCustomWidgetMenuSelect\">" +
                this.label + "<br />" + "<div class=\"ClientCustomWidgetMenuSelectValue\">Empty ...</div></div>";
        }
    ...
    class MenuTextInput extends MenuItem {
        ...
        redraw(jsonObject) {
            this.element.innerHTML = "<i class=\"fa fa-undo\" aria-hidden=\"true\"></i><i class=\"fa fa-floppy-o\" aria-hidden=\"true\"></i>" +
                this.label +
                "<br />" + "<div class=\"" + this.input_value_class_name + "\">Empty ...</div>";
        }
        ...
        enableEditMode(jsonObject) {
            ...
            this.value == "" ?
                menu_value_element.html("<input class=\"form-control\" placeholder=\"Virtual Machine Name\" type=\"text\">") :
                menu_value_element.html("<input class=\"form-control\" placeholder=\"Virtual Machine Name\" type=\"text\" value=" + this.value + ">");
        }
    ...
    rap.registerTypeHandler("com.intrinsec.zeus.utils.ui.widgets.CustomWidgetMenuTextInput", {
        factory: function(properties) {
            return new MenuTextInput(
                properties,
                REMOTE_EVENT_CLICK_SAVE_BUTTON,
                REMOTE_EVENT_CLICK_UNDO_BUTTON,
                MENU_INPUT_VALUE_CLASS_NAME
            );
        },
        destructor: function(widget) {},
        methods: [
            'redraw',
            REMOTE_METHOD_ENABLE_EDIT_MODE,
            REMOTE_METHOD_DISABLE_EDIT_MODE
        ],
        events: [
            REMOTE_EVENT_CLICK_SAVE_BUTTON,
            REMOTE_EVENT_CLICK_UNDO_BUTTON
